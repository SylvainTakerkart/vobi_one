...
class BlkFile:
	...
	def get_4d_image(self, temporal_binning=1, spatial_binning=1):
		...
		self.get_head() # Reads each field of BLK file's header and save them into a dictionnary
		self.get_data() # Reads the data contained in the BLK file

		# Recuperation of image attributes
		x_size=self.header['framewidth']
		y_size=self.header['frameheidth']
		z_size=1
		t_size=self.header['nframesperstim']

		a = np.reshape(self.data,(t_size,z_size,y_size,x_size)) # Transformation of data linear bitstream to a regular image 2D + time data
		...
		if spatial_binning > 1:
			x_size_binned = int( np.ceil( float(x_size) / spatial_binning ) )
			y_size_binned = int( np.ceil( float(y_size) / spatial_binning ) )
			b = np.zeros( [ t_size_binned, 1, y_size_binned, x_size_binned] )
			for x in range(x_size_binned):
				ind_min_x = x * spatial_binning
				ind_max_x = min( (x+1) * spatial_binning, x_size)
				for y in range(y_size_binned):
