...
# The blank division. Divides the averaged blank datas to image

from neuroProcesses import *
from oi_formats import list_formats
import os

name = _t_('Blank Division')
...
def validation( self ):
    ...
    try:
        ...
        raise ValidationError(_t_('Impossible to import oitrial_processes')) # Raises an exception
...
signature=Signature(
    ...
    "data", ReadDiskItem( 'OI 2D+t Image' ,list_formats ), # Input image path
    "average_file", ReadDiskItem( 'OI 2D+t Blank Mean' , list_formats ), # The averaged raw image
    "bkd_data", WriteDiskItem( 'OI BkSD' , list_formats ), # The path of the image after blank division
    )
...
def initValuesbksd( self , inp ):
    ...
    if self.data is not None:
        ...
        value['filename_variable']= os.path.splitext( os.path.splitext(os.path.basename( self.data.fullPath() ))[0])[0]+ '_bkd'
...
def initialization( self ):
    ...
    self.signature["bkd_data"].browseUserLevel = 2 # Browse only visible for expert user

    self.addLink( "average_file", "data",self.initValuesAverage) # Change on average_file if change on data

    self.addLink( "bkd_data", "format", self.initValuesbksd) # Change on bks_data if change on format
    self.addLink( "bkd_data", "firstlevel_analysis", self.initValuesbksd) # Change on bks_data if change on firstlevel_analysis
    self.addLink( "bkd_data", "data", self.initValuesbksd) # Change on bks_data if change on data
...
def execution( self,context ):
    ...
    attributes = self.bkd_data.hierarchyAttributes() # Attributes recuperation


    format =  os.path.splitext(os.path.splitext(self.bkd_data.fullPath())[0])[1]+os.path.splitext(self.bkd_data.fullPath())[1] # Format recuperation

    # The blank division. Divides the averaged blank datas to image
    oitrial_processes.blank_division_process(
	path_data=self.data.fullPath(), # Data file path
        path_average=self.average_file.fullPath(), # The averaged raw image
	path_bksd=self.bkd_data.fullPath(), # The path of the image after blank substraction
